...
Entry.prototype.retry = function () {
  this.failed = false;
};
...
function PlanState(planNames) {
  ...
  // keep track of any failures so we don't need to search all entries for failures
  this.failed = 0;
}
...
PlanState.prototype.retry = function (name) {
  this._apply('retry', name);
  this.failed--;
};
...
PlanState.prototype.allDone = function () {
  var result;
  if (!this.failed) {
    result = this.entriesArray.every(function (entry) {
      return entry.isDone();
    });
  } else {
